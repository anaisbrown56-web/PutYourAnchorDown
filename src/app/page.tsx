...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    fetch(`/api/locations?university=${encodeURIComponent(selectedUni)}`)
      .then((r) => r.json())
      .then((locations: Location[]) => {
        const categories = ['CAMPUS_SPOT', 'RESTAURANT', 'ATTRACTION']
        const featured = categories.map(
          (cat) => locations.find((l) => l.category === cat) || null
        )
              ...
              <div className="relative">
                <div className="flex items-center">
                  <div className="relative flex-1">
                    <svg
                      className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-silver-400"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (selectedUni && e.target.value !== selectedUni) {
                          setSelectedUni('')
                          setFeaturedLocations([])
                        }
                      }}
                      ...
                      className="w-full pl-12 pr-10 py-4 border-2 border-silver-200 rounded-xl text-navy-800 text-lg placeholder-silver-400 focus:outline-none focus:ring-2 focus:ring-gold-400 focus:border-gold-400 transition-all shadow-sm"
                    />
                    {selectedUni && (
                      <button
                        onClick={clearSelection}
                        className="absolute right-3 top-1/2 -translate-y-1/2 text-silver-400 hover:text-navy-700 transition-colors"
                      >
                ...
                {/* Dropdown */}
                {showDropdown && !selectedUni && searchQuery.length > 0 && (
                  <div className="absolute z-20 w-full mt-2 bg-white border border-silver-200 rounded-xl shadow-lg overflow-hidden">
                    {filteredUniversities.length > 0 ? (
                      filteredUniversities.map((uni) => (
                        <button
                          key={uni}
                          onClick={() => selectUniversity(uni)}
                          className="w-full text-left px-4 py-3 hover:bg-gold-50 text-navy-800 font-medium transition-colors border-b border-silver-100 last:border-0 flex items-center gap-3"
                        >
                      ...
                      <div className="px-4 py-3 text-silver-500 text-sm">
                ...
                {/* Quick select when empty */}
                {showDropdown && !selectedUni && searchQuery.length === 0 && universities.length > 0 && (
                  <div className="absolute z-20 w-full mt-2 bg-white border border-silver-200 rounded-xl shadow-lg overflow-hidden">
                    <div className="px-4 py-2 text-xs font-semibold text-silver-400 uppercase tracking-wide bg-silver-50">
      ...
      {selectedUni && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="text-center mt-10">
            <Link
              href={`/locations?university=${encodeURIComponent(selectedUni)}`}
